...
class ConditionsTest {
    ...
    @Test
    fun `BEFORE operator works for dates`() {
        val condition = Condition.Plain(
            attributeKey = "date",
            operator = BEFORE,
            value = ConditionValue.DateTimeValue(Instant.parse("2023-10-04T00:00:00Z")),
        )

        Conditions.conditionIsMatched(
            condition = condition,
            context = mapOf("date" to AttributeValue.DateValue(Instant.parse("2022-11-04T00:00:00Z")))
        ) shouldBe true

        Conditions.conditionIsMatched(
            condition = condition,
            context = mapOf("date" to AttributeValue.DateValue(Instant.parse("2024-10-04T00:00:00Z")))
        ) shouldBe false
    }

    @Test
    fun `AFTER operator works for dates`() {
        val condition = Condition.Plain(
            attributeKey = "date",
            operator = AFTER,
            value = ConditionValue.DateTimeValue(Instant.parse("2023-10-04T00:00:00Z")),
        ...
        Conditions.conditionIsMatched(
            condition = condition,
            context = mapOf("date" to AttributeValue.DateValue(Instant.parse("2022-10-04T00:00:00Z")))
        ) shouldBe false
    ...
    fun `multiple conditions work`() {
        ...
        val beforeCondition = Condition.Plain(
            attributeKey = "date",
            operator = BEFORE,
            value = ConditionValue.DateTimeValue(Instant.fromEpochMilliseconds(1632307200000)),
        )
        ...
        val context = mapOf(
            ...
            "date" to AttributeValue.DateValue(Instant.parse("2023-10-04T00:00:00Z")), // false
