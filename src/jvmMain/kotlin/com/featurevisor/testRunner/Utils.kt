...
fun getContextValue(contextValue: Any?) =
    when (contextValue) {
        ...
        is String -> {
            // Try to parse as ISO-8601 date first, fallback to string
            try {
                val instant = Instant.parse(contextValue)
                AttributeValue.DateValue(instant)
            } catch (e: Exception) {
                AttributeValue.StringValue(contextValue)
            }
        }
        is Instant -> AttributeValue.DateValue(contextValue)
