...
import com.featurevisor.types.TestFeature
...
private fun mapMatrixValues(value: Any) =
    when(value){
        ...
        is String -> {
            // Try to parse as ISO-8601 date string, fallback to regular string
            try {
                val instant = Instant.parse(value)
                AttributeValue.DateValue(instant)
            } catch (e: Exception) {
    ...
    return when (value) {
        ...
        is String -> {
            if (value.equals("", true)) {
                ...
                // Try to parse as date first (ISO-8601 format)
                try {
                    val instant = Instant.parse(value)
                    AttributeValue.DateValue(instant)
                } catch (dateException: Exception) {
                    // If not a date, try parsing as number, then fallback to string
...
private fun parseCondition(conditionData: Any?): Condition {
    return when (conditionData) {
        ...
        is List<*> -> {
            val conditionsList = conditionData
